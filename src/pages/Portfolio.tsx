...
const AnalyticsBadges = ({ meta }: { meta: SoftwareMeta }) => (
  <div className="flex flex-wrap gap-1.5 mt-2">
    ...
    {meta.page_load_ms     != null && <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-amber-500/10 text-amber-300 font-mono text-xs"><BarChart2 size={10} /> {meta.page_load_ms}ms</span>}
    {meta.monthly_visitors != null && <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-amber-500/10 text-amber-300 font-mono text-xs"><Users size={10} /> {meta.monthly_visitors.toLocaleString()}/mo</span>}
    {(meta.tech_stack ?? []).length > 0 && <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-amber-500/10 text-amber-300 font-mono text-xs"><Code size={10} /> {meta.tech_stack!.slice(0, 2).join(", ")}{meta.tech_stack!.length > 2 ? ` +${meta.tech_stack!.length - 2}` : ""}</span>}
  </div>
);
const ArtBadges = ({ meta }: { meta: ArtMeta }) => (
  <div className="flex flex-wrap gap-1.5 mt-2">
    {meta.medium     && <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-rose-500/10 text-rose-300 font-mono text-xs"><PenTool size={10} /> {meta.medium.charAt(0) + meta.medium.slice(1).toLowerCase().replace(/_/g, " ")}</span>}
    {meta.dimensions && <span className="px-1.5 py-0.5 rounded-full bg-rose-500/10 text-rose-300 font-mono text-xs">{meta.dimensions}</span>}
    {meta.year       && <span className="px-1.5 py-0.5 rounded-full bg-rose-500/10 text-rose-300 font-mono text-xs">{meta.year}</span>}
    {meta.price != null && <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-rose-500/15 text-rose-200 font-mono text-xs font-semibold">${meta.price}</span>}
    {meta.is_available && <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-emerald-500/15 text-emerald-300 font-mono text-xs"><span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" /> Available</span>}
  </div>
);
const DesignBadges = ({ meta }: { meta: DesignMeta }) => (
  <div className="flex flex-wrap gap-1.5 mt-2">
    {(meta.software ?? []).map(sw => <span key={sw} className="px-1.5 py-0.5 rounded-full bg-cyan-500/10 text-cyan-300 font-mono text-xs">{sw}</span>)}
    {meta.client_name && <span className="px-1.5 py-0.5 rounded-full bg-cyan-500/10 text-cyan-300 font-mono text-xs">{meta.client_name}</span>}
    {meta.year        && <span className="px-1.5 py-0.5 rounded-full bg-cyan-500/10 text-cyan-300 font-mono text-xs">{meta.year}</span>}
  </div>
);
const PhotographyBadges = ({ meta }: { meta: DesignMeta }) => (
  <div className="flex flex-wrap gap-1.5 mt-2">
    {(meta.software ?? []).map(eq => <span key={eq} className="px-1.5 py-0.5 rounded-full bg-violet-500/10 text-violet-300 font-mono text-xs">{eq}</span>)}
    {meta.client_name && <span className="px-1.5 py-0.5 rounded-full bg-violet-500/10 text-violet-300 font-mono text-xs">{meta.client_name}</span>}
    {meta.year        && <span className="px-1.5 py-0.5 rounded-full bg-violet-500/10 text-violet-300 font-mono text-xs">{meta.year}</span>}
  </div>
);
            ...
            {filters.map(f => {
              ...
              return (
                <button key={f} onClick={() => setActive(f)}
                  className={`px-4 py-1.5 rounded-full font-mono text-xs uppercase tracking-wider transition-all inline-flex items-center gap-1.5 ${
                    active === f
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        {project.featured && (
                          <div className="absolute top-2 left-2 px-1.5 py-0.5 rounded-lg bg-primary/80 backdrop-blur-sm font-mono text-xs text-primary-foreground flex items-center gap-1">
                        ...
                        <div className="absolute inset-0 bg-background/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center p-4 text-center">
                          ...
                          <p className="text-muted-foreground font-body text-sm mb-3 max-w-xs line-clamp-2">{project.description}</p>
                          ...
                          <div className="flex gap-1.5 mb-3 flex-wrap justify-center">
                            {project.tags.slice(0, 3).map(tag => <span key={tag} className={`px-2 py-0.5 rounded-full ${style.badge} font-mono text-xs`}>{tag}</span>)}
                          </div>
        ...
        {lightbox !== null && currentProject && (() => {
          ...
          return (
            <motion.div
              ...
              className="fixed inset-0 z-[80] bg-background/90 backdrop-blur-xl flex items-center justify-center p-3 md:p-6"
              onClick={() => setLightbox(null)}
            >
              <button className="absolute top-4 right-4 text-foreground hover:text-primary z-10" onClick={() => setLightbox(null)}>
                <X size={24} />
              </button>
              ...
              >
                ...
                <div className="md:w-3/5 flex-shrink-0 relative bg-background/50">
                  ...
                  {currentProject.images.length > 1 && (
                    <>
                      {lightbox.imageIdx > 0 && (
                        <button onClick={() => navigateImage(-1)} className="absolute left-2 top-1/2 -translate-y-1/2 p-1.5 rounded-full bg-background/70 backdrop-blur-sm text-foreground hover:bg-background/90">
                          <ChevronLeft size={18} />
                        </button>
                      )}
                      {lightbox.imageIdx < currentProject.images.length - 1 && (
                        <button onClick={() => navigateImage(1)} className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-full bg-background/70 backdrop-blur-sm text-foreground hover:bg-background/90">
                      ...
                      <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1.5">
                        {currentProject.images.map((_, idx) => (
                          <button key={idx} onClick={() => setLightbox({ ...lightbox, imageIdx: idx })}
                            className={`w-1.5 h-1.5 rounded-full transition-all ${idx === lightbox.imageIdx ? "bg-primary scale-125" : "bg-foreground/30"}`}
                          />
                        ))}
                      </div>
                    </>
                ...
                <div className="p-4 md:p-5 flex flex-col justify-between md:w-2/5 overflow-y-auto">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <CI size={13} className={cs.accent} />
                    ...
                    <h2 className="font-display text-xl font-bold text-foreground mt-1 mb-2">{currentProject.title}</h2>
                    <p className="text-muted-foreground font-body text-sm leading-relaxed mb-3">{currentProject.description}</p>

                    {currentProject.category === "Web Dev" && currentProject.softwareMeta && (
                      <div className="mb-3 p-2.5 rounded-xl bg-amber-500/5 border border-amber-500/20">
                        <p className="font-mono text-xs text-amber-400 uppercase mb-1.5 flex items-center gap-1"><BarChart2 size={10} /> Analytics</p>
                        ...
                        {currentProject.softwareMeta.analytics_note && <p className="text-muted-foreground font-body text-xs mt-1.5 leading-relaxed">{currentProject.softwareMeta.analytics_note}</p>}
                      </div>
                    )}
                    {currentProject.category === "Fine Art" && currentProject.artMeta && (
                      <div className="mb-3 p-2.5 rounded-xl bg-rose-500/5 border border-rose-500/20">
                        <p className="font-mono text-xs text-rose-400 uppercase mb-1.5">Artwork Details</p>
                    ...
                    {currentProject.category === "Design" && currentProject.designMeta && (
                      <div className="mb-3 p-2.5 rounded-xl bg-cyan-500/5 border border-cyan-500/20">
                        <p className="font-mono text-xs text-cyan-400 uppercase mb-1.5">Design Details</p>
                        <div className="flex flex-wrap gap-1.5">
                          {(currentProject.designMeta.software ?? []).map(sw => <span key={sw} className="px-1.5 py-0.5 rounded-full bg-cyan-500/10 text-cyan-300 font-mono text-xs">{sw}</span>)}
                          {currentProject.designMeta.client_name && <span className="px-1.5 py-0.5 rounded-full bg-cyan-500/10 text-cyan-300 font-mono text-xs">Client: {currentProject.designMeta.client_name}</span>}
                        </div>
                      </div>
                    )}
                    {currentProject.category === "Photography" && currentProject.designMeta && (
                      <div className="mb-3 p-2.5 rounded-xl bg-violet-500/5 border border-violet-500/20">
                        <p className="font-mono text-xs text-violet-400 uppercase mb-1.5 flex items-center gap-1"><Camera size={10} /> Photography Details</p>
                        <div className="flex flex-wrap gap-1.5">
                          {(currentProject.designMeta.software ?? []).map(eq => <span key={eq} className="px-1.5 py-0.5 rounded-full bg-violet-500/10 text-violet-300 font-mono text-xs">{eq}</span>)}
                          {currentProject.designMeta.client_name && <span className="px-1.5 py-0.5 rounded-full bg-violet-500/10 text-violet-300 font-mono text-xs">Series: {currentProject.designMeta.client_name}</span>}
                          {currentProject.designMeta.year        && <span className="px-1.5 py-0.5 rounded-full bg-violet-500/10 text-violet-300 font-mono text-xs">{currentProject.designMeta.year}</span>}
                        </div>
                      </div>
                    )}

                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {currentProject.tags.map(tag => <span key={tag} className={`px-2 py-0.5 rounded-full ${cs.badge} font-mono text-xs`}>{tag}</span>)}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-1.5">
                    {currentProject.links.map((link, li) => (
                      <a key={li} href={link.url} target="_blank" rel="noopener noreferrer"
                        className={`inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl font-mono text-xs uppercase tracking-wider ${cs.bg} ${cs.accent} border ${cs.border} hover:scale-[1.02] transition-transform`}>
                    ...
                    {currentProject.category === "Fine Art" && currentProject.artMeta?.shop_url && currentProject.artMeta.shop_url !== "#" && !currentProject.links.some(l => l.link_type === "shop") && (
                      <a href={currentProject.artMeta.shop_url} target="_blank" rel="noopener noreferrer"
                        className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl font-mono text-xs uppercase tracking-wider bg-rose-500/10 text-rose-300 border border-rose-500/30 hover:scale-[1.02] transition-transform">
                    ...
                    {currentProject.category === "Web Dev" && currentProject.softwareMeta?.live_url && !currentProject.links.some(l => l.link_type === "live") && (
                      <a href={currentProject.softwareMeta.live_url} target="_blank" rel="noopener noreferrer"
                        className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl font-mono text-xs uppercase tracking-wider bg-amber-500/10 text-amber-300 border border-amber-500/30 hover:scale-[1.02] transition-transform">
                    ...
                    {currentProject.category === "Photography" && currentProject.designMeta?.behance_url && !currentProject.links.some(l => l.link_type === "live") && (
                      <a href={currentProject.designMeta.behance_url} target="_blank" rel="noopener noreferrer"
                        className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl font-mono text-xs uppercase tracking-wider bg-violet-500/10 text-violet-300 border border-violet-500/30 hover:scale-[1.02] transition-transform">
